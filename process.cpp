...
bool processproc()
{
	...
	for (i = 0; i < num; i++)
	{
		...
		else { printf("您要产生的进程消费者，它是第%d个进程。\n", p->numlabel); }
...
bool waitempty()
{
	if (empty <= 0)
	{
		printf("进程%d：缓冲区存数，缓冲区满，该程进入生产者等待队列\n", exe->numlabel);
...
bool waitfull()
{
	if (full <= 0)
	{
		printf("进程%d：缓冲区取数，缓冲区空，该程进入消费者等待队列\n", exe->numlabel);
...
void signalfull()
{
	...
	if (hasElement(consumerhead)) {
		...
		linkqueue(p, &readytail);
		printf("等待的消费者进程金进入就绪队列，的进程号是%d\n", p->numlabel);
	}
	full++;
}
void producerrun()
